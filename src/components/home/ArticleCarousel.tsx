...
export function ArticleCarousel({ articles }: { articles: Article[] }) {
  ...
  return (
    ...
    <div data-reveal className="relative flex w-full items-center lg:gap-5" {...pause}>
      <button
        type="button"
      ...
      >
        {articles.map((article) => (
          <Link
            ...
            className="group relative block h-[170px] w-full shrink-0 snap-start overflow-hidden rounded-[10px] transition-transform duration-150 ease-[var(--ease-out-quart)] sm:h-[180px] sm:w-[300px] hover:-translate-y-[3px] active:translate-y-0"
          >
            <Asset
              src={article.image}
              alt={article.title}
